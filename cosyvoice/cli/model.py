...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if stream is True:
            token_offset = 0
            stream_token_hop_len = self.token_hop_len
            prompt_token_pad = int(np.ceil(flow_prompt_speech_token.shape[1] / stream_token_hop_len) * stream_token_hop_len - flow_prompt_speech_token.shape[1])
            while True:
                time.sleep(0.1)
                this_token_hop_len = stream_token_hop_len + prompt_token_pad if token_offset == 0 else stream_token_hop_len
                if len(self.tts_speech_token_dict[this_uuid]) - token_offset >= this_token_hop_len + self.flow.pre_lookahead_len:
                    ...
                    stream_token_hop_len = min(self.token_max_hop_len, int(stream_token_hop_len * self.stream_scale_factor))
                    yield {'tts_speech': this_tts_speech.cpu()}
